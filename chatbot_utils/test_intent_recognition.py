...
def _load_dialogs_with_source(dialogs_dir: str) -> List[DialogWithSource]:
...
def _load_test_sets(dialogs: List[DialogWithSource]) -> List[DialogIntentTestSet]:
    test_sets: List[DialogIntentTestSet] = []

    for wrapped in dialogs:
        dialog = wrapped.dialog
        test_file_path = _resolve_test_file_path(wrapped.file_path, dialog.name)
        ...
        try:
            with open(test_file_path, "r", encoding="utf-8") as file:
                raw = json.load(file)
            cases = _parse_test_cases(raw, default_expected_intent=dialog.name)
        except Exception as exc:
            logger.warning("Invalid test JSON for dialog %s at %s: %s", dialog.name, test_file_path, exc)
            continue
...
def _extract_detected_intent(response: Any, extractor: IntentExtractor) -> Tuple[Optional[str], Optional[Dict[str, Any]]]:
    metadata_items = getattr(response, "messages_metadata", None) or []
...
def run_intent_tests(dialogs_dir: str) -> IntentTestReport:
    dialogs = _load_dialogs_with_source(dialogs_dir)
    test_sets = _load_test_sets(dialogs)
    ...
    for test_set in test_sets:
        for index, case in enumerate(test_set.cases):
            ...
            try:
                ...
                detected_intent, metadata_raw = _extract_detected_intent(response, extractor)
            except Exception as exc:
            ...
            passed = (
                ...
                and _normalize_intent(detected_intent) == _normalize_intent(expected_intent)
            )
